...
TARGET_TABLE = "swapi_demo_1.raw_data_1.characters"
...
def fetch_all_people(start_url: str) -> List[Dict[str, Optional[str]]]:
    ...
    while next_url:
        ...
        if isinstance(payload, list):
            rows.extend(normalize_person(p) for p in payload if isinstance(p, dict))
            break

        if isinstance(payload, dict):
            results = payload.get("results", [])
            if isinstance(results, list):
                rows.extend(normalize_person(p) for p in results if isinstance(p, dict))
            else:
...
def main() -> None:
    ...
    spark.sql("CREATE CATALOG IF NOT EXISTS swapi_demo_1")
    spark.sql("CREATE SCHEMA IF NOT EXISTS swapi_demo_1.raw_data_1")
    spark.sql(
        """
        CREATE TABLE IF NOT EXISTS swapi_demo_1.raw_data_1.characters (
          name STRING,
          height STRING,
          mass STRING,
          birth_year STRING,
    ...
    df = spark.createDataFrame(rows).select("name", "height", "mass", "birth_year", "homeworld")
